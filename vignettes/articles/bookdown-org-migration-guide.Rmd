...
-   Consider using the Connect Cloud GitHub integration to have your source code pulled directly from your GitHub repository, either manually triggered from the Connect Cloud dashboard or automatically upon pushing to a branch. See <https://docs.posit.co/connect-cloud/user/publish/github.html>

-   Keep using the publish_book() function while setting render = “server”

    ``` r
    bookdown::publish_book(render = 'server')
    ```

In both cases, your book will now be rendered directly on the Connect Cloud server.

If you require a specific configuration for rendering your document, refer to the Connect Cloud documentation. For example, for environment variable: <https://docs.posit.co/connect-cloud/user/publish/github.html#advanced-settings>
...
There are a few options, depending on your desired workflow.
...
-   If you commit your rendered book output on Github (e.g. the `_book` directory), then you can use the Static Document deployment
    ...
    ![Static Deployment Icon on Connect Cloud publish page](images/static-deploy.png)

    When configuring this deployment, select the rendered index.html as Primary file. Example:
...
The last option is to use R Markdown deployment on Connect Cloud.

![](images/rmarkdown-deploy.png)

This will allow you to keep only the source of your book in GitHub, not the rendered HTML, and have it rendered on the Connect Cloud server. This approach requires you to specify what R packages Connect Cloud needs to use, which is done with a `manifest.json` file generated by the **rsconnect** package. 

See "[Q: Can I have my book rendered on Connect Cloud?]" above on how to set that up.
