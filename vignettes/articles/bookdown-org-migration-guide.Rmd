...
### Q: I don’t want to migrate, but I want to download my current book hosted on bookdown.org. Can I do that? {#download-bundle}
...
Yes. `bookdown::render_book()` has produced for you at the time a static website, and it can be hosted anywhere that supports such content. It can be deployed to Connect Cloud, but you could also consider Netlify, GitHub Pages, or any other static page hosting service. Below, we are guiding you to republish on Connect Cloud.
...
To deploy a folder of static HTML to Connect Cloud using **rsconnect**:

``` r
# First, authenticate with Connect Cloud (one-time setup)
rsconnect::connectCloudUser()

# Deploy your static HTML folder
rsconnect::deployApp(
  appDir = "path/to/your/html/folder",  # folder containing index.html
  appName = "my-book-name",             # URL-friendly name
  appTitle = "My Book Title",      # Optional title, otherwise appName.
  appMode = "static",
  contentCategory = "site",
  server = "connect.posit.cloud" # should be the default one used.
)
```

To download a bundle of your previously deployed book to bookdown.org, see [Q: I don’t want to migrate, but I want to download my current book hosted on bookdown.org. Can I do that?](#download-bundle)

Or you can use the `connectapi` package, but it requires [getting an API key](https://docs.posit.co/connect/user/api-keys/):

``` r
# Set your bookdown.org API key (get it from https://bookdown.org/connect/)
Sys.setenv(CONNECT_API_KEY = "your-api-key")

client <- connectapi::connect(server = "https://bookdown.org")
content <- connectapi::content_item(client, guid = "your-app-guid")
connectapi::download_bundle(content, filename = "my-book.tar.gz")

# Extract
untar("my-book.tar.gz", exdir = "my-book")
```

Then you can use `rsconnect::deployApp()` as above.
