...
- Fixed `html_document2()` breaking Plotly (and other htmlwidgets) in self-contained mode on Windows, caused by `^Z` (`\x1a`) bytes in embedded JavaScript being treated as end-of-file by `readLines()` in text mode (thanks, @aomop, #1523).
