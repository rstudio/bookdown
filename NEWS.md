...
- Disabling `sharing` in config with `no`, `false` or `null` now works and is no more impacted by check in config for Google+ sharing removal (thanks, @lwjohnst86, #812).
