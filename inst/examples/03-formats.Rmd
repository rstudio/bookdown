...
The third button is for font/theme settings. You can change the font size (bigger or smaller), the font family (serif or sans serif), and the theme (`White`, `Sepia`, or `Night`). These settings can be changed via the `fontsettings` option. The button can be removed from the toolbar by setting `fontsettings: null` (or `no`).
