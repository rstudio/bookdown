...
The `bs4_book()` output format is built with Bootstrap (<https://getbootstrap.com>), using carefully crafted features to provide a clean reading experience whether you are on a phone, tablet, or desktop. The basic layout includes three columns of content so readers can quickly see the book's table of contents on the left-hand side, and a chapter's contents in the sidebar on the right. You can read an example book here: <https://mastering-shiny.org/>
...
You need to use the following syntax to create them: 
...
If these values are set, then the associated `<meta>` tags will be set in the
resulting HTML file. The provided values will be used as is in `index.html`.
