...
- the Bootstrap 4 style (Section \@ref(bs4-book)), 
- the Bootstrap style (Section \@ref(bootstrap-style)), and 
...
The easiest way to get started writing a new `gitbook` is to use the RStudio Project Wizard (*File > New Project > New Directory > Book project using bookdown*) and select `gitbook` from the dropdown menu: 

```{r fig.align='center', echo=FALSE, fig.alt="Screenshot of the RStudio Project Wizard for creating a new gitbook project."}
knitr::include_graphics('images/gitbook.png', dpi = NA)
```

If you do not use RStudio or prefer a function, you can create the same project template with `bookdown::create_gitbook()` from your R console. See `?create_gitbook` for help.
...
The `bs4_book()` output format is built with Bootstrap (<https://getbootstrap.com>), using carefully crafted features to provide a clean reading experience whether you are on a phone, tablet, or desktop. The layout includes three columns of content so readers can quickly see the book chapters as links on the left-hand side, and within-chapter content links in the right-hand sidebar. You can read an example book here: <https://mastering-shiny.org/>

```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), fig.link='https://mastering-shiny.org/', fig.alt="Home page for a bs4_book showing the layout with a table of contents on the left, main chapter content in the center, and an 'on this page' sidebar on the right."}
knitr::include_graphics('images/bs4_book.png', dpi = NA)
```

This output format builds upon the Bootstrap CSS framework (version 4: <https://getbootstrap.com/docs/4.0/>), an open source library of reusable chunks of HTML, CSS, and JavaScript code.
...
This format also requires extra dependencies that you'll be prompted to install: `r knitr::combine_words(bookdown:::bs4_book_deps())`.

#### Writing a `bs4_book`

The easiest way to get started writing a new `bs4_book` is to use the RStudio Project Wizard (*File > New Project > New Directory > Book project using bookdown*) and select `bs4_book` from the dropdown menu: 

```{r fig.align='center', echo=FALSE, fig.alt="Screenshot of the RStudio Project Wizard for creating a new bs4_book project."}
knitr::include_graphics('images/new-bs4-book.png', dpi = NA)
```

If you do not use RStudio or prefer a function, you can create the same project template with `bookdown::create_bs4_book()` from your R console. See `?create_bs4_book` for help.

This theme is designed for books that use one chapter per page. 
This means that each chapter is an `.Rmd` file, and each `.Rmd` file can contain one (and only one) chapter. 
A chapter *must* start with a first-level heading: `# A chapter`, and can contain one (and only one) first-level heading. 
...
The first- and second-level headings will appear in that chapter's sidebar on the upper right as a "sticky" within-chapter table of contents that stays in place as you scroll down. In the sidebar, when a section is navigated to, subheadings will auto-expand such that third-level headings like "An even shorter section" are visible.
...
This output format allows for easy customization of colors and fonts with the **bslib** package (<https://pkgs.rstudio.com/bslib/>).
...
For custom font settings, adding a `google:` keyword triggers [`sass::font_google()`’s](https://rstudio.github.io/sass/reference/font_face.html) ability to automatically import Google Font files (<https://fonts.google.com/>). By default, `google:` will bundle font files with your book, so that when you share it with someone else, the fonts are guaranteed to render, even without an internet connection (`local: false` imports files via URL instead of serving them locally). Here is an example YAML that changes the `base_font`, `heading_font`, and `code_font`:
...
The `bs4_book` theme also includes special callout blocks with predefined styles, which will include a colored border around the text and/or code inside the callout. Use the following syntax to create a callout block: 
...
You may use Markdown syntax and inline code inside a block. When knitted, this will render as:

```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), fig.alt="A special rmd-note callout block."}
...
For LaTeX output, only the content of these blocks will be shown with no colored outline as for HTML. It is up to the user to define the appearance of these blocks for LaTeX output using custom environments. See this section in The R Markdown Cookbook: <https://bookdown.org/yihui/rmarkdown-cookbook/custom-blocks.html> for a how-to.

#### HTML metadata

Bookdown will generate HTML `<meta>` tags based on Pandoc's variables set in `index.Rmd`, described in \@ref(metadata-for-sharing). Additionally, `bs4_book()` will create unique chapter descriptions auto-generated from the chapter's contents. You can have a look at [`bs4_book()` HTML
...
The `bs4_book` format makes footnotes appear inline when you click on them. 
Making your citations _footnotes_
allows readers to read them near the text they are referred in.
To do that, download a footnote style CSL file
(e.g. chicago-fullnote-bibliography.csl; we recommend: https://www.zotero.org/styles/), then
...
The result will be that your bibliography will be split and relevant citation items put at the bottom of each chapter, so that readers do not have to navigate to a different bibliography page to see the details of citations.
...
To use another icon, specify it with the correct prefix (`fas`, `fab`, ...) ([Font Awesome 5](https://fontawesome.com/v5.0/how-to-use/on-the-web/referencing-icons/basic-use)).
...
### Two-column Bootstrap style {#bootstrap-style}
