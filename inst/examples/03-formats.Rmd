...
```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), fig.alt="Screenshot of the RStudio Project Wizard for creating a new gitbook project."}
knitr::include_graphics('images/gitbook.png', dpi = NA)
```

If you don't use RStudio or prefer a function, you can create the same project template with `bookdown::create_gitbook()` from your R console. See `?create_gitbook` for help.
...
### Bootstrap 4 style {#bs4-book}

The `bs4_book()` output format is built with Bootstrap (<https://getbootstrap.com>), using carefully crafted features to provide a clean reading experience whether you are on a phone, tablet, or desktop. You can read an example book here: <https://mastering-shiny.org/>

```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), fig.link='https://mastering-shiny.org/', fig.alt="Screenshot of a bs4_book landing page to show the basic site layout"}
knitr::include_graphics('images/bs4_book.png', dpi = NA)
```

This output format builds upon the Bootstrap CSS framework. Bootstrap is a library of reusable chunks of HTML, CSS, and JavaScript code that you can call on to modify your book without writing that code yourself. It is a free and open source project, and you can view the repository on GitHub: <https://github.com/twbs/bootstrap>. Bootstrap versions do get updated periodically; this format uses version 4 (<https://getbootstrap.com/docs/4.0/>).
...
- Built-in search (broken down by section) that helps readers quickly find t
...
- R syntax highlighting and autolinking by
  the downlit package (<https://downlit.r-lib.org/>) is paired with a accessible
...
The output format function in **bookdown** is `bs4_book()`. Here are its arguments:
...
```{r fig.align='center', echo=FALSE, include=identical(knitr:::pandoc_to(), 'html'), fig.alt="Screenshot of the RStudio Project Wizard for creating a new bs4_book project."}
knitr::include_graphics('images/new-bs4-book.png', dpi = NA)
```

If you don't use RStudio or prefer a function, you can create the same project template with `bookdown::create_bs4_book()` from your R console. See `?create_bs4_book` for help.
...
A chapter *must* start with a first-level heading: `# A good chapter`, and can contain one (and only one) first-level heading. 
...
The `bs4_book` theme also includes special callout blocks with predefined styles. Available blocks are: `.rmdnote`, `.rmdcaution`, `.rmdimportant`, `.rmdtip`, and `.rmdwarning`. 

You need to use the following syntax to use them: 
...
For LaTeX output, only the content of these blocks will be shown with no special styling as for HTML. It is up to the user to define the appearance of these blocks for LaTeX output using custom environments. See this section in The R Markdown Cookbook: <https://bookdown.org/yihui/rmarkdown-cookbook/custom-blocks.html> for a how-to.


#### HTML metadata

HTML `<meta>` tags will be set based on Pandoc's variables set in `index.Rmd`:
 ...
 * `cover-image` is the path to your local cover image, and will be used as
 `og:image`, appended to `url` value. `cover-image` can also take an absolute url.
 * `twitter-handle` will be used as `twitter:site` content.

If these values are set, then the associated `<meta>` tag will be set in the
resulting HTML file. The provided values will be used asis in `index.html`.
They will then be tweaked for each chapter to produce different metadata for h
...
template](https://github.com/rstudio/bookdown/blob/master/inst/templates/bs4_book.html) for details on how these variables are used.
...
icon](https://fontawesome.com), and otherwise a GitLab Font Awesome icon.
