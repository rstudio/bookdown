...
1. Install the RStudio IDE ([the preview version](https://www.rstudio.com/products/rstudio/download/preview/) is recommended) if you have not done so;
...
1. Bring up the `Build` tab of RStudio IDE. Follow the `More` menu and click to `Configure Build Tools`. Set `Project build tools` to `Website`. 
...
At the moment, there are three output formats that you may use: `bookdown::pdf_book`, `bookdown::gitbook`, and `bookdown::html_book`. There is a `bookdown::render_book()` function similar to `rmarkdown::render()`, but it was designed to render _multiple_ Rmd documents into a book using the output format functions. You may either call this function from command line, or use it in the RStudio IDE. Here are some command line examples:
