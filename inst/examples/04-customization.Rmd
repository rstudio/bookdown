...
The `appendix_counters` field should be a list of strings, which are used for the first part of appendix counters. For example, e.g., `['1st', '2nd', '3rd']` will result in appendices with titles like `'1st 1 X'`, `'1st 2 X'`, `'2nd Y'`). You can also set `appendix counters` as a function that takes a vector of numbers. This vector represents the section number, subsection number, and so on for an appendix. The function must return a single character string. For example, if `appendix_counters` is a function that replaces all numbers with English words, the appendix titles `'1.1 X'`, `'1.2 X'`, and `'2 Y'` would become `'One.One X'`, `'One.Two X'`, and `'Two Y'`. Note that appendix numbers start with 1, no matter how many regular chapters came before them.
