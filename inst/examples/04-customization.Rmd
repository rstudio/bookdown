...
We have mentioned `rmd_files` in Section \@ref(usage), and there are more (optional) settings you can configure for a book in `_bookdown.yml`\index{\_bookdown.yml}^[For the [`bs4_book`](#bs4-book) format, the edit, history, and view pages are specified with the [repo](#specifying-the-repository) element.]:
