...
Go to Netlify Drop ([netlify.com/drop](https://app.netlify.com/drop)) where you should see a box like this, as shown in Figure \@ref(fig:netlify-drop).

```{r netlify-drop, echo=FALSE, fig.align='center', fig.alt="Screenshot of drag & drop deploy box on Netlify"}
knitr::include_graphics("images/netlify-drop.png")
```

Then drag and drop the output directory from your **bookdown** project (`_book/` by default, unless you changed this in your `_bookdown.yml` file). You should see your book deploy quickly with a random subdomain name of the form `https://random-word-12345.netlify.com`.
