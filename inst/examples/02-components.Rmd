...
If a readers try to try to access a page in your book that cannot be found (e.g a link to a page that no more exists), a browser will display a [404 error](https://en.wikipedia.org/wiki/HTTP_404) as it can’t find the requested web page. This 404 error is displayed on a 404 page that each browser has a default for. You may want to customized this page to be in the style of your HTML book. 
Most web serving platforms (e.g. Netlify, Github Pages, or Gitlab Pages) will use a file named `404.html` in the root of your website as a custom error page if you provide it. 

With HTML book format, **bookdown** creates a default `404.html`  using simple content (a header, and a body of 2 paragraphs).  

```{r 404-page, fig.alt='Screenshot of an example 404 page for an HTML book built with bs4_book.', fig.align='center', echo=FALSE}
knitr::include_graphics('images/404.png')
```

To customize the 404 page instead of using the default, you may add either a `_404.Rmd` or a `_404.md` file to your project root. If either file is found when you render the book, the content will be rendered as an `html_fragment()` and will be included as the body of the 404 page. This means that the overall structure of your book's website (including navbar, footer, sidebars, etc.) and the CSS styling will be preserved on the 404 page.

If a 404.html file already exists in the main repo at the root level with the `.Rmd` files, then **bookdown** will leave that file as is and will not overwrite it. This is because we assume you already have a mechanism in place in your publishing workflow to use this custom `404.html` you had setup.
