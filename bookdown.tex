...
## [1] knitr_1.52      shiny_1.14.0    htmltools_0.5.9
## [4] rmarkdown_2.32  bookdown_0.48.1 miniUI_0.1.2   
## [7] tools_4.6.1
\end{verbatim}
...
If you prefer the old \textbf{webshot} package, you need to install it together with PhantomJS, and set the chunk option \texttt{webshot\ =\ \textquotesingle{}webshot\textquotesingle{}} to force \textbf{knitr} to use it:
...
Third, the screenshot package has some options to control the automatic screenshots, and you may specify these options via the chunk option \texttt{screenshot.opts}, which takes a list like \texttt{list(delay\ =\ 2)}. See the help page \texttt{?webshot2::webshot} (or \texttt{?webshot::webshot}) for the full list of possible options. Here the \texttt{delay} option can be important for widgets that take long time to render: \texttt{delay} specifies the number of seconds to wait before the screenshot is taken. If you see an incomplete screenshot, you may want to specify a longer delay (the default is 0.2 seconds).
...
\caption{Home page of a book with the three-column Bootstrap style.}\label{fig:unnamed-chunk-13}
\end{figure}
...
\FunctionTok{date}\KeywordTok{:}\AttributeTok{ }\StringTok{"2026{-}09{-}18"}
...
\DocumentationTok{\#\# [1] \textquotesingle{}3.11\textquotesingle{}}
\end{Highlighting}
\end{Shaded}
